...
class ServiceInformation(models.Model):
    _name = 'service.information'
    _description = 'Información de servicio'
    ...
    conectividad_pop = fields.Many2one('res.partner', string='POP', domain="[('type','=','site'), ('ref','=','POP')]")
    ...
    cloud_numeros_extensiones = fields.One2many('service.information.cloud', 'service_information_cloud_id', string='Numeros')
    ...
    cloud_contacto_pago = fields.Many2one('res.partner', string='Contacto pago')
    cloud_contacto_tecnico = fields.Many2one('res.partner', string='Contacto técnico')
    cloud_certificado_ssl = fields.Char(string='Certificado SSL')
    cloud_resolucion = fields.Selection([
        ('local', 'Local'),
        ('externa', 'Externa'),
        ], string='Resolución')
